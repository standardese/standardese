...
translation_unit parser::parse(const char* path, const compile_config& c) const
{
    ...
    const char* basic_args[] = {"-x", "c++", "-I",
                                STANDARDESE_DETAIL_STRINGIFY(LIBCLANG_SYSTEM_INCLUDE_DIR)};

    std::vector<const char*> args(basic_args,
                                  basic_args + sizeof(basic_args) / sizeof(const char*));
