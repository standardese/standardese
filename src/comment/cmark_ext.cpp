...
namespace
{
void init_node(cmark_syntax_extension* self, cmark_node* node, unsigned raw_cmd, const char* arg)
{
...
struct inline_predicate_lambda
{
    ...
    bool operator()(cmark_node* node)
    {
        ...
        else if (!last_line || std::abs(last_line.value() - cmark_node_get_start_line(node)) <= 1)
        {
...
cmark_node* postprocess_nodes(cmark_syntax_extension* self, cmark_node* cur, Predicate process_node)
{
    auto first     = cur;
    ...
    while (process_node(cur))
    {
        ...
        else if (need_brief.try_reset() && cmark_node_get_type(cur) == CMARK_NODE_PARAGRAPH)
        {
            ...
            auto brief = make_node(self, node_section(), unsigned(section_type::brief), nullptr);
...
cmark_syntax_extension* standardese::comment::detail::create_command_extension(config& c)
{
    ...
    auto can_contain
        = [](cmark_syntax_extension*, cmark_node* node, cmark_node_type child_type) -> int {
    ...
    cmark_syntax_extension_set_open_block_func(ext,
                                               [](cmark_syntax_extension* self, int indented,
                                                  cmark_parser*  parser,
                                                  cmark_node*    parent_container,
                                                  unsigned char* input, int len) -> cmark_node* {
                                                   // if we have a command character,
                                                   // create an inline node consuming as much as
                                                   // possible, then return a paragraph containing
                                                   // this command
                                                   ...
                                                   auto cur = real_input;
                                                   auto node
                                                       = parse_node(self, cur, parser,
                                                                    parent_container, indented);
                                                   cmark_parser_advance_offset(parser,
                                                                               reinterpret_cast<
                                                                                   char*>(input),
                                                                               static_cast<int>(
                                                                                   cur
