...
namespace standardese { namespace detail
{
    ...
    bool is_blacklisted(const parser &par, const basic_cpp_entity_ref<Kind> &ref)
    {
        ...
        if (!target)
            return false;
        return par.get_output_config().get_blacklist().is_blacklisted(entity_blacklist::synopsis, *target);
    }
    ...
    inline bool is_blacklisted(const parser &par, const cpp_type_ref &ref)
    {
        ...
        if (!entity)
            return false;
        return par.get_output_config().get_blacklist().is_blacklisted(entity_blacklist::synopsis, *entity);
    }
    ...
    void write_type_value_default(const parser &par,
    ...
    void write_bases(const parser &par, output_base::code_block_writer &out, const cpp_class &c, bool extract_private);

    void write_parameters(const parser &par,
